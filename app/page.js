...
function VisitingDesk({ creature, cursor, onBeginAdoption }) {
  ...
  return <section className="visitingSection" id="desk" style={{"--pet":creature.color}}>
    ...
    <div className="desk" ref={deskRef} onPointerMove={interact}>
      ...
      {curiosity&&<div key={`${creature.id}-${curiosity.id}`} className={`curiosityScene scene-${curiosity.id} visitor-${creature.id}`}><button onClick={()=>setActiveCuriosity(null)} aria-label="Put curiosity away">×</button><div className="sceneGlow"/><img className="sceneObject" src={asset(curiosity.image)} alt={curiosity.label}/><div className="creatureReaction"><i/><img className="sceneCreature" src={asset(creature.image)} alt={`${creature.name} interacting with the ${curiosity.label.toLowerCase()}`}/><span>{creature.name}'s live reaction</span></div><article><small>FIELD EVENT / {curiosity.id.toUpperCase()}</small><h3>{curiosity.title}</h3><p>{curiosity.detail}</p><b>{creature.name.toUpperCase()} IS INVESTIGATING</b></article>{curiosity.id==="window"&&<div className="fieldStars"><i/><i/><i/><i/></div>}</div>}
      <div className="restingSpot"><span>OPTIONAL<br/>RESTING SPOT</span><i/></div>
      <CreatureFollower creature={creature} cursor={cursor} reacting={reaction} active={!curiosity}/>
